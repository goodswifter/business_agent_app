...
class RealNameProvider with ChangeNotifier {
  ...
  List get dataList => this._dataList;
  List get enterpriseDataList => this._enterpriseDataList;
  String get businessLicenseImageStr => this._businessLicenseImageStr;
  String get openingPermitImageStr => this._openingPermitImageStr;
  ...
  void updateImageData(
    ...
    this._businessLicenseImageStr = businessLicenseImageStr;
    this._openingPermitImageStr = openingPermitImageStr;
  ...
  void updateInfoData(UserInfoModel userItemModel) async {
    ...
    if (currentType == "C") {
      this._dataList[0]["value"] = "个人";
    } else {
      this._dataList[0]["value"] = "企业";
    }
    ...
    userExtsList.forEach((element) {
      ...
      if (userExts.type == DetermineUserRealNameStatusUtils.C_0101) {
        //用户名
        this._dataList[1]["value"] = userExts.value;
      } else if (userExts.type == DetermineUserRealNameStatusUtils.C_0102) {
        //身份证号
        this._dataList[2]["value"] = userExts.value;
      }
    });
  ...
  void updateEnterpriseInfoData(UserInfoModel userItemModel) async {
    ...
    if (currentType == DetermineUserRealNameStatusUtils.TYPE_C) {
      //"C"
      this._enterpriseDataList[0]["value"] = "个人";
    } else {
      this._enterpriseDataList[0]["value"] = "企业";
    }
    ...
    //用户实名状态   U:初始未认证  S:实名审核通过   W:认证提交未审核  F:用户销毁  R:实名审核拒绝
    if (currentStatus == "S") {
      this._enterpriseDataList[6]["value"] = "已实名";
    } else if (currentStatus == "P") {
      this._enterpriseDataList[6]["value"] = "认证中";
    } else if (currentStatus == "R") {
      this._enterpriseDataList[6]["value"] = "实名审核拒绝";
    } else if (currentStatus == "W") {
      this._enterpriseDataList[6]["value"] = "认证提交未审核";
    }
    ...
    userExtsList.forEach((element) {
      ...
      if (userExts.type == DetermineUserRealNameStatusUtils.ENTERPRISE_0201) {
        this._enterpriseDataList[1]["value"] = userExts.value; //"企业名称";
      } else if (userExts.type ==
          DetermineUserRealNameStatusUtils.ENTERPRISE_0202) {
        this._enterpriseDataList[2]["value"] = userExts.value; //"营业执照号";
      } else if (userExts.type ==
          DetermineUserRealNameStatusUtils.ENTERPRISE_0203) {
        this._enterpriseDataList[3]["value"] = userExts.value; // "企业性质";
      } else if (userExts.type ==
        ...
        if (userExts.value == null ||
          ...
          this._enterpriseDataList[4]["value"] = userExts.value; //"开户许可证";
        }
      } else if (userExts.type ==
        ...
        if (userExts.value == null ||
          ...
          this._enterpriseDataList[5]["value"] = userExts.value; //"开户许可证";
        }
      }
    });
