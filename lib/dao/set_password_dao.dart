...
class SetPasswordDao {
  ///设置登录密码接口
  static Future<Map<String, dynamic>?> doLoginPasswwordData(
      Map paramsMap) async {
  ...
  ///1、设置支付密码接口、
  ///2、更新图片信息接口
  static Future<Map<String, dynamic>?> doPayPasswwordData(Map paramsMap) async {
    PublishSubject<ValidateResult> resultSubject =
        PublishSubject<ValidateResult>();
  ...
  ///修改支付密码接口
  static Future<Map<String, dynamic>?> doModifyPayPasswwordData(
      Map paramsMap) async {
    PublishSubject<ValidateResult> resultSubject =
        PublishSubject<ValidateResult>();
    ...
    ValidateResult result = await HttpManager.instance
        .request(await Api.apiModifyPayPasswordData(paramsMap));
